...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
  ...
  #pragma omp parallel
  {
    ...
    for(int i=this_thread; i<ARRAY_SIZE; i+=thread_num)
      z[i] = x[i] + y[i];
  }
}
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
  ...
  #pragma omp parallel
  {
    ...
    int div_task = ARRAY_SIZE / thread_num;

    for(int i=this_thread * div_task; i<(this_thread+1)*div_task; i++)
      z[i] = x[i] + y[i];
    
    if (this_thread + 1 == thread_num) {
      int i = (this_thread+1)*div_task;
      while (i < ARRAY_SIZE) {
        z[i] = x[i] + y[i];
        i++;
      }
