...
class TestRelu(TestCase):
    ...
    def test_short(self):
        ...
        t.input_scalar("a1", 4)
        t.call("relu")
        t.check_array(array0, [1, 0, 3, 0, 5, -6, -7])
        t.execute()

    def test_long_arr(self):
        t = AssemblyTest(self, "relu.s")
        arr = [(-1)**(i % 2) * i for i in range(0xff)]
        array0 = t.array(arr)
        t.input_array("a0", array0)
        t.input_scalar("a1", len(arr))
        t.call("relu")
        t.check_array(array0, [max(i, 0) for i in arr])
        t.execute()
